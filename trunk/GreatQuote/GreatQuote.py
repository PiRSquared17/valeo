# -*- coding: utf-8 -*-
'''

== FEATURES ==
* Configurations:
    1. Minutos to change quote
    2. Your language
* System start with Registry (Registry/HKCU/Run)

== TODO ==
* Randon background color contrast with text color
* Replace old image

== DEPENDENCIES ==
* Microsoft Windows
* Python 2.5
* Python Imaging Library
* PyWin32 API

== PORTUGUESE QUOTES ==
"Homens demoram a perceber que perderam o amor da mulher, por pior que a tenham tratado." - Arthur Conan Doyle
"Ninguém deve cometer a mesma tolice duas vezes. A possibilidade de escolha é muito grande." - Jean Paul Sartre
"O silêncio é reacionário."  - Jean Paul Sartre
"No manicômio global, entre um senhor que julga ser Maomé e outro que acredita ser Buffalo Bill, entre o terrorrismo dos atentados e o terrorrismo da guerra, a violência está nos arruinando". - Eduarno Galeano
"A primeira condição para modificar a realidade consiste em conhecê-la." - Eduardo Galeano
"Tenho saudades de um país que ainda não existe no mapa." - Eduardo Galeano
"Espero que a pessoa que entre nas minhas exposições não seja a mesma ao sair". - Sebastião Salgado

== ENGLISH QUOTES ==
"The highest morality may prove also to be the highest wisdom when the half-told story comes to be finished." - Arthur Conan Doyle
"Imagination is not an empirical or superadded power of consciousness, it is the whole of consciousness as it realizes its freedom."  - Jean Paul Sartre

'''
